...
if($_POST['cat_id']!='null')       //获取默认显示的分类
{
    $where = @$_POST['cat_id'];
    $sql1 = "select * from goods_info where cat_id = $where";	//获得goods_info分类后的所有数据 
    $sql2 = "select * from goods_info where cat_id = $where limit $mutiple,$limit";
}
else{
    $sql1 = "select * from goods_info";	//获得goods_info的所有数据
    $sql2 = "select * from goods_info limit $mutiple,$limit";
}
