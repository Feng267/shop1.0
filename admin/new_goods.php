...
<?php
    ...
    if(isset($_POST["sub"]))
    {
        ...
        //从数据库里获得 商品类名 ，再填进去
        $sql = "select cat_name as name from goods_info where cat_id = $cat_id limit 1";
        $result = mysqli_query($conn,$sql)or die("错误描述: " . mysqli_error($conn));
        $cat_name = mysqli_fetch_assoc($result);
        $cat_name = $cat_name['name'];

        //判断商品名是否输入，没有则默认为类ID
        if(!empty(@$_POST["goods_name"]))     //商品名
        {
        ...
        $file = @$_FILES['file'];  //商品图片
        $date = date("Y-m-d H:i:s");  //时间不输入，默认为中国时间

        //判断是否上传了图片
        if($file['name'] != null){
            $name = $file['name'];  //获得上传的文件
            $type = strtolower(substr($name,strrpos($name,'.'))); //得到文件类型，并且都转化成小写
            ...
            echo "<script>alert('你还没上传图片');</script>";
            die ;
        }
        
        $pic_name = date("Y-m-d").'-'.$goods_id.$type;
        $pic_path = "../image/goods_pic/".$pic_name; //图片实际存储的路径
        $true_path = "image/goods_pic/".$pic_name;//存放到数据库里的图片路径
        
        //如果下列都为真，则向数据库添加新内容
        if($cat_id and $goods_name and isset($goods_price) and $goods_number)
        {
            //如果图片移动成功，则向数据库添加新内容
            if(move_uploaded_file($file['tmp_name'],$pic_path))
            {
                $sql = "INSERT INTO goods_info (goods_id,cat_id,cat_name,goods_name,goods_price,goods_number,goods_small_logo,date) VALUES($goods_id,$cat_id,'$cat_name','$goods_name',$goods_price,$goods_number,'$true_path','$date')";
                mysqli_query($conn,$sql)or die("错误描述: " . mysqli_error($conn)); 
                if(mysqli_affected_rows($conn))
                {
                    echo "<script>alert('新商品添加啦');</script>";
                }
                else
                {
                    echo "<script>alert('新商品添加失败');</script>";
                }
            }
            else
            {
